...
class Coupon(models.Model):
    ...
    used_customers=models.ManyToManyField(Customer,related_name="used_coupons",blank=True)
    ...
    def is_valid(self,customer):
        if timezone.localtime()>self.expire:
            return [False,_("The time of this coupon is in the past")]
        elif self.used_customers.count()>=self.limit:
            return [False,_("The limit of this coupon is over")]
        elif self.used_customers.contains(customer):
            return [False,_("You have already used this coupon")]
        else:
            return [True,_("This coupon was used successfully")]
